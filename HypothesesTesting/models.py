...
class Subsession(BaseSubsession):
    ...
    def group_by_arrival_time_method(self, waiting_players):
        ...
        for p in waiting_players:
            ...
            if time.time() - p.participant.vars["arrivalTime"] > 5*1: # adjust time?
...
class Group(BaseGroup):
    ...
    def condition_picker(self):
        ...
        # pick group condition ------------------------------------------------
        # check which condition is to be picked (condition counter)
        if self.session.vars["groupConditionCounter"] == 0:
            # pick the condition at this position in the list
            pickedCondition=itemgetter(0)(self.session.vars["groupConditionsList"])
            # increase counter by one so the next pick is the next item in the list
            self.session.vars["groupConditionCounter"]+=1

            for p in players:
                # give participants nickname
                p.nickname = "Teilnehmer*in {}".format(p.id_in_group)
                # save picked condition
                p.conditionHypothesis = pickedCondition

        # check which condition is to be picked (condition counter)
        else:
            # pick the condition at this position in the list
            pickedCondition=itemgetter(1)(self.session.vars["groupConditionsList"])
            # reset condition counter
            self.session.vars["groupConditionCounter"]=0
            # shuffle condition counter again
            random.shuffle(self.session.vars["groupConditionsShuffeld"])
        ...
        # assign group condition to group
        self.groupCondition = pickedCondition
